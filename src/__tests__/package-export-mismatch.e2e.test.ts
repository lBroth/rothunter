...
describe('package-export-mismatch — full RotHunter pipeline', () => {
  it(
    'surfaces a missing publish target in the final findings',
    async () => {
      ...
      try {
        ...
        const hits = result.findings.filter(
          (f) => f.detectorId === 'package-export-mismatch',
        );
  ...
  it(
    'stays silent when every target resolves to a source counterpart',
    async () => {
      const root = await setup({
        'package.json': JSON.stringify({
          name: 'demo',
