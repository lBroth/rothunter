...
describe('schema-shape-divergence — full RotHunter pipeline', () => {
  it(
    'surfaces a User / UserDTO drift in the final findings',
    async () => {
  ...
  it(
    'stays silent when two unrelated types share only one common field',
    async () => {
      const root = await setup({
        'src/user.ts':
          'export interface User {\n' +
  ...
  it(
    'is silent on exact-match pairs (duplicate-type owns that case)',
    async () => {
      const root = await setup({
        'src/a.ts':
          'export interface User {\n' +
