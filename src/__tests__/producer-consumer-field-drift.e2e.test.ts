...
describe('producer-consumer-field-drift — full RotHunter pipeline', () => {
  it('surfaces a server-reads / client-misses drift in the final findings', async () => {
    ...
    try {
      ...
      const hits = result.findings.filter((f) => f.detectorId === 'producer-consumer-field-drift');
  ...
  it('stays silent when every server-read field is in the client body', async () => {
    const root = await setup({
      'src/server.ts':
        'declare const app: any;\n' +
        "app.post('/api/users', (req: any, res: any) => {\n" +
  ...
  it('does NOT flag a server route with no client callsite (cannot make a claim)', async () => {
    const root = await setup({
      'src/server.ts':
        'declare const app: any;\n' +
