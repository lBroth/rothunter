...
describe('env-var-undeclared — full RotHunter pipeline', () => {
  it(
    'surfaces an undeclared env var read in the final findings',
    async () => {
  ...
  it(
    'does NOT flag when .env.example declares the variable',
    async () => {
      const root = await setup({
        'src/index.ts': "export const url = process.env.DATABASE_URL;\n",
  ...
  it(
    'emits a paired dead-env finding for vars declared but never read',
    async () => {
      const root = await setup({
