...
describe('test-without-assertion — full RotHunter pipeline', () => {
  it('surfaces an assertion-free test in the final findings list', async () => {
    ...
    try {
      ...
      const hits = result.findings.filter((f) => f.detectorId === 'test-without-assertion');
  ...
  it('does NOT flag a real assertion-bearing test', async () => {
    const root = await setup({
      'src/lib.ts': 'export function add(a: number, b: number): number { return a + b; }\n',
      'src/__tests__/lib.test.ts':
  ...
  it('leaves `.skip` / `.only` / `.todo` to the skip-tests detector', async () => {
    const root = await setup({
    ...
    try {
      const rothunter = new RotHunter();
      const result = await rothunter.run({ workspaceRoot: root });
      const twa = result.findings.filter((f) => f.detectorId === 'test-without-assertion');
