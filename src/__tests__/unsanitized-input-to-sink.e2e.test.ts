...
describe('unsanitized-input-to-sink — full RotHunter pipeline', () => {
  it(
    'surfaces a SQL-injection-shaped flow in the final findings',
    async () => {
      ...
      try {
        ...
        const hits = result.findings.filter(
          (f) => f.detectorId === 'unsanitized-input-to-sink',
        );
  ...
  it(
    'surfaces a command-injection-shaped flow via exec()',
    async () => {
      const root = await setup({
        'src/handler.ts':
          "declare const app: any;\n" +
  ...
  it(
    'stays silent on a parameterised query that uses no tainted name',
    async () => {
      const root = await setup({
        'src/handler.ts':
          "declare const app: any;\n" +
